...
mcd $d/b
rvm use 3.1.3@versions-conf-b --create --versions-conf --install
rvm current             # match=/^ruby-3.1.3@versions-conf-b$/
[[ -f .versions.conf ]] # status=0
...
rvm 3.1.3 do rvm --force gemset delete versions-conf-b
rm -rf $d
